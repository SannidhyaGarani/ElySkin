...
const Toast = ({ message, type, onClose }) => (
  <motion.div
    ...
    exit={{ opacity: 0, x: 20 }}
    className="fixed top-6 right-6 z-[10000] flex items-center gap-3 bg-[#0A2D50] text-white px-6 py-4 rounded-2xl shadow-2xl border border-white/10"
  >
    ...
    >
      <div className="relative aspect-square mb-8 bg-[#FFF9F1] rounded-[32px] overflow-hidden group-hover:scale-[0.98] transition-transform duration-700">
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-500">
          <button 
            ...
            className="p-4 bg-white rounded-2xl text-[#0A2D50] hover:bg-[#FF6B00] hover:text-white transition-all shadow-xl cursor-pointer disabled:opacity-50"
          >
            {loadingType === 'wishlist' ? <Loader2 size={20} className="animate-spin" /> : <Heart size={20} />}
          </button>
          <button 
            onClick={(e) => { e.stopPropagation(); setSelectedProduct(product); }}
            className="p-4 bg-white rounded-2xl text-[#0A2D50] hover:bg-[#FF6B00] hover:text-white transition-all shadow-xl cursor-pointer"
          >
            <Eye size={20} />
          </button>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#FF6B00]">{product.category || 'Skin Care'}</span>
          <div className="flex items-center gap-1">
            <span className="text-[10px] font-black text-[#0A2D50]">★ {product.rating || '4.8'}</span>
          </div>
        </div>
        
        <h3 className="text-xl font-black text-[#0A2D50] leading-tight group-hover:text-[#FF6B00] transition-colors">{product.name}</h3>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-[#0A2D50]/5">
          ...
          <button 
            ...
            className="w-14 h-14 bg-[#0A2D50] text-white rounded-2xl flex items-center justify-center hover:bg-[#FF6B00] hover:scale-110 transition-all shadow-xl cursor-pointer disabled:bg-[#0A2D50]/50"
          >
...
const ProductGrid = () => {
  ...
  return (
    <>
      ...
      <section className="relative py-20 bg-[#FFF9F1] overflow-hidden cursor-none">
        <CustomCursor />
        
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 relative z-10">
          <div className="flex flex-col items-center text-center mb-24 space-y-6">
            <h2 className="text-5xl md:text-7xl lg:text-8xl text-[#0A2D50] font-serif leading-[0.9]">
              Curated <i className="font-light">Essentials</i>
            </h2>
            
            <div className="flex flex-wrap justify-center gap-2 mt-12 border-b border-[#0A2D50]/5 pb-4">
              {['All Products', 'Oils', 'Cosmetics', 'Sets'].map((cat) => (
                <button
                  key={cat}
                  onClick={() => setActiveCategory(cat)}
                  className={`px-6 py-3 text-sm font-bold tracking-widest uppercase cursor-pointer interactive transition-colors ${activeCategory === cat ? 'text-[#0A2D50]' : 'text-[#0A2D50]/40'}`}
                >
                  {cat}
                </button>
              ))}
